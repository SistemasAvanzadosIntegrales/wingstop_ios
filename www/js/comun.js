        ...
        $.ajax({
            ...
            success:function(resp) {
                ...
                if( resp.error == '' ) {
                    ...
                    localStorage.setItem('tarjetaClave', resp.tarjetaClave);
            ...
            if ( data.length ) {
                ...
                $("#plate-addons input[type='checkbox']").change(function() {
                    ...
                    if ( data[0].Cantidad > 0 ) {
                        if ( $(this).is( ":checked" ) ) {
                            $("#plate-addons-count").append('<section class="col-6 float-left no-pl"><label class="form-check-label"><input class="form-control-text" type="text" name="count-'+inputHow+'" value="0" id="count-'+inputHow+'">'+nameLabel+'</label></section>');
                        }
                        else {
...
function setPedido(idToken) {
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            localStorage.setItem('promotionsAll', '');
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            if ( data.length == 0 ) {
                ...
                $.each(data, function(i, item) {
                    $( "#menu-categorias ul" ).append( "<li class='col-12'><figure>"+(item.Imagen ? item.Imagen : imageDefault)+"</figure><div class='categorias-descripcion'><article><p class='descripcion'>"+item.Nombre+"</p></article></div><div class='btn-siguiente text-right'><a href='#' onclick=\"goPlate('"+item.Consecutivo+"');\"><i class='fa fa-chevron-right'></i></a></div></li>" );
                });
            }
...
function getPromotions() {
    ...
    $.ajax({
        ...
        success:function(data){
            $.each(data, function(i, item) {
                $( "#menu-categorias ul" ).append( "<li class='col-12'><figure>"+imageDefault+"</figure><div class='categorias-descripcion'><article><p class='descripcion'>"+item.Nombre+"</p></article></div><div class='btn-siguiente text-right'><a href='#' onclick=\"goPromotion('"+item.Promocion+"', '"+item.Promocion+"');\"><i class='fa fa-chevron-right'></i></a></div></li>" );
            });
        },
        error: function(XMLHttpRequest, textStatus, errorThrown) {
            console.log("Status: " + textStatus);
